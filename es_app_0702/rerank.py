...
class Reranker:
    ...
    def rerank_combined_chunk(self, json_data: List[Dict[str, str]], response: str, processor_threshold=0.7, docai_threshold=0.7) -> Response:
        """
        第一次重排：生成的结果先和聚合的 chunk 比对，保证 reference 的正确性；
        第二次重排：生成的结果再和 docai 的 chunk 比对，保证图片的正确性。
        """
        chunk_texts = [item['chunk_text'] for item in json_data]
        scores = self.model.predict(response, chunk_texts)
        combined = list(zip(json_data, scores))
        ...
        if not filtered_combined:
            sorted_combined = sorted(combined, key=lambda x: x[1], reverse=True)
            logger.info(f"没有超过阈值的数据，使用原始数据进行排序！")
        else:
            sorted_combined = sorted(filtered_combined, key=lambda x: x[1], reverse=True)
    ...
    def rerank_combined_chunk_new(self, json_data: List[Dict[str, str]], response: str, processor_threshold=0.7, docai_threshold=0.7) -> Response:
        """
        第一次重排：生成的结果先和聚合的 chunk 比对，保证 reference 的正确性；
        第二次重排：生成的结果再和 docai 的 chunk 比对，保证图片的正确性。
        """
        def filter_and_sort(items, scores, threshold):
            combined = list(zip(items, scores))
            filtered_combined = [item for item in combined if item[1] >= threshold]
            return sorted(filtered_combined, key=lambda x: x[1], reverse=True)

        chunk_texts = [item['chunk_text'] for item in json_data]
        scores = self.model.predict(response, chunk_texts)
        
        sorted_combined = filter_and_sort(json_data, scores, processor_threshold)
        
        if not sorted_combined:
            logger.info("没有超过阈值的数据，返回空列表！")
            return Response(json.dumps([], ensure_ascii=False, indent=4), mimetype='application/json')
        ...
        for chunk_data in sorted_json_data:
            if 'text_list' in chunk_data and 'positions' in chunk_data and 'base64_list' in chunk_data:
                ...
                scores = self.model.predict(response, text_list)
                sorted_combined = filter_and_sort(zip(text_list, positions, base64_list), scores, docai_threshold)

                if sorted_combined:
                    chunk_data['text_list'], chunk_data['positions'], chunk_data['base64_list'] = zip(*sorted_combined)
                else:
                    chunk_data['text_list'] = []
                    chunk_data['positions'] = []
                    chunk_data['base64_list'] = []
