            ...
            for index, chunk in enumerate(self.data):
                ...
                if chunk['type'] == 'TABLE':
                    ...
                    try:
                        ...
                        if len(split_text) > 1 and split_text[1] == '问答\n答案':
                            remove_index = index
                            logger.info(f"第1步")
                            text_list = split_text[2:]
                            logger.info(f"第2步")
                            for iner_index, text in enumerate(text_list):
                                aux_chunk = copy.deepcopy(chunk)
                                aux_chunk['text'] = '问答\t答案' + "\n" + text
...
if __name__ =="__main__":
    ...
    file_path = '/root/web_demo/HybirdSearch/es_app_0702/ZeRO_ Memory Optimizations Toward Training Trillion Parameter Models.pdf.json'
    file_name = 'ZeRO_ Memory Optimizations Toward Training Trillion Parameter Models'   # 别带.pdf
    dp = KnowledgeDocumentPreprocessor(tokenizer_path,  file_path, file_name)
    knowledge, long_content = dp.process()
    
    with open('/root/web_demo/HybirdSearch/es_app_0702/ZeRO_ Memory Optimizations Toward Training Trillion Parameter Models.json', 'w', encoding='utf-8') as f:
        json.dump(knowledge, f, ensure_ascii=False, indent=4)
    with open('/root/web_demo/HybirdSearch/es_app_0702/ZeRO_ Memory Optimizations Toward Training Trillion Parameter Models_long.json', 'w', encoding='utf-8') as f:
