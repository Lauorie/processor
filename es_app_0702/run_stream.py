...
def handle_search_and_streaming(user_id, query, index_type, search_more, history, filenames=None):
    try:
        ...
        sorted_json_data = service.reranker.rerank_combined_chunk_new(json_data, answer)
        # sorted_json_data 是 Response 数据，要换为 JSON 格式
        json_data_reranked = json.loads(sorted_json_data.get_data(as_text=True))
        # 如果 json_data_reranked 不为空:
        if json_data_reranked:
            # 只取第一组
            base_64_list = []
            if isinstance(json_data_reranked[0]["base64_list"], list):
                base_64_list.extend(json_data_reranked[0]["base64_list"])
            else:
                base_64_list.append(json_data_reranked[0]["base64_list"])
